...
public String icon_status;
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        CheckBoxPreference circle_switch = (CheckBoxPreference) findPreference("circle_switch");
...
//            });
        ...
        if(key.equals("circle_switch"))
        {   Intent intent = new Intent("impulse.extras.showcirclebattery");
            if(line_switch.isChecked())
            {
                Settings.System.putInt(getContext().getContentResolver(),"impulse_circlebattery_show",1);
            }
            else
            {
                Settings.System.putInt(getContext().getContentResolver(),"impulse_circlebattery_show",0);
            }
            getContext().sendBroadcast(intent);

        }
        if(key.equals("circle_width"))
        {

            final SeekBar seekBar = new SeekBar(getContext());
            seekBar.setMax(50);
            seekBar.setMin(1);
            seekBar.setProgress((int) Settings.System.getFloat(getContext().getContentResolver(),"impulse_circle_width",10.0f),true);
            final AlertDialog.Builder dialog = new AlertDialog.Builder(getContext());
            seekBar.setOnSeekBarChangeListener(new android.widget.SeekBar.OnSeekBarChangeListener() {
                @Override
                public void onProgressChanged(android.widget.SeekBar seekBar, int progress, boolean fromUser) {
                    ToastUtil toastUtil = new ToastUtil();
                    toastUtil.showMsg(getContext(),"当前选择的宽度为：" + String.valueOf(progress)+"dp");
                    Settings.System.putFloat(getContext().getContentResolver(),"impulse_circle_width",progress);
                    Intent intent = new Intent("impulse.extras.changecircle");
                    getContext().sendBroadcast(intent);
                }
            ...
            dialog.setTitle("环形电池宽度").setView(seekBar);
            dialog.setCancelable(true);
            dialog.setNegativeButton("还原", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
        ...
        if(key.equals("circle_color"))
        {
            ColorPickerDialog.Builder colorPickerDialog = new ColorPickerDialog.Builder(getContext(), AlertDialog.THEME_LIGHT_EDIT);
            colorPickerDialog.setTitle("请选择环形电池颜色")
                    .setPreferenceName("Line_Battery")
                    .setPositiveButton("选择",
                            new ColorEnvelopeListener() {
                                @Override
                                public void onColorSelected(ColorEnvelope envelope, boolean fromUser) {
                                    //Settings.System.putInt(getContext().getContentResolver(),"impulse_line_color",-16711936);
                                    Settings.System.putInt(getContext().getContentResolver(),"impulse_circle_color",envelope.getColor());
                                    Intent intent = new Intent("impulse.extras.changecircle");
                                    getContext().sendBroadcast(intent);
                                }
                            })
        ...
        if(key.equals("power_percent"))
        {
            ...
            {
                Settings.System.putInt(contentResolver,"status_bar_show_percent",8);
            }
            else
            {
                Settings.System.putInt(contentResolver,"status_bar_show_percent",0);
            ...
            ContentResolver contentResolver = getContext().getContentResolver();
            ...
            else
              {
                  ...
                  else
                  {
                      mount();
                      ShellUtils.execCommand("chmod 664 /system/priv-app/MiuiSystemUI/MiuiSystemUI.apk",true);
                      ShellUtils.execCommand("cd /system/xbin/Ming/MiuiSystemUI/default\n/system/xbin/zip /system/priv-app/MiuiSystemUI/MiuiSystemUI.apk res/layout/*",true);
                      ShellUtils.execCommand("chmod 644 /system/priv-app/MiuiSystemUI/MiuiSystemUI.apk",true);
                  }
